...
For fleet deployments, these variables can be migrated incrementally to a validated [runtime TOML file](/self-hosting/runtime-configuration). Environment variables continue to override TOML for compatibility.
...
| `DB_CONNECT_TIMEOUT` | Optional (defaults to `10s`) | PostgreSQL startup connection timeout. Accepts `1s` through `1m`. |
| `DB_TLS_MODE` | Optional | PostgreSQL TLS policy: `disable`, `require`, `verify-ca`, or `verify-full`. Use `verify-full` in production. |
| `DB_TLS_ROOT_CERT` | Required with verified TLS | Absolute path to the PostgreSQL CA bundle when `DB_TLS_MODE` is `verify-ca` or `verify-full`. |
