...
<AccordionGroup>
  ...
  <Accordion title="agent-vault owner bridge">
    ...
    The command refuses a non-loopback listener, a non-HTTP or non-loopback public origin, missing explicit ports, non-HTTPS Agent Vault targets, and configurations without `client.workload_api`. See [SPIFFE-only human administration](/self-hosting/spiffe-migration#human-administration-after-the-switch) for the Kubernetes security boundary and OAuth callback registration.
  </Accordion>
