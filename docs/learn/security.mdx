...
- **TLS**: Production fleets use `database.tls_mode = "verify-full"` with an explicit `database.tls_root_cert`; local-only Compose may use `sslmode=disable`.
- **Access controls**: The runtime role owns only the Agent Vault database and schema. It has DDL ownership for startup migrations but no cluster-administration privileges.
- **Advisory locks**: Service-config writes use `pg_advisory_lock` to prevent concurrent conflicting updates across instances.

For production fleets, set `database.tls_mode = "verify-full"` with an explicit `database.tls_root_cert`. Use a dedicated login that owns only the Agent Vault database and schema: startup migrations need DDL ownership, but never `SUPERUSER`, `CREATEDB`, `CREATEROLE`, replication, or bypass-RLS privileges. Bound total database demand as `replicas × database.max_open_conns`, leaving capacity for operators, monitoring, migrations, and failover overlap.
